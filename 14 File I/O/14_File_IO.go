...
// Reading files requires checking for and handling errors.
...
func main() {
	...
	// For these tasks, start by `Open`ing a file to obtain an `os.File` value.
	...
	// Close the file when you're done (usually this is scheduled immediately after `Open`ing with `defer`).
	f.Close()
	...
	// `bufio` provides buffered writers in addition to the buffered readers we saw above.
	w := bufio.NewWriter(f)
